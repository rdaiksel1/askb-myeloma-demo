...
export default function MyelomaDemo() {
  ...
  const handleDiseaseChipClick = () => {
    if (demoStepRef.current === 1 && !isAnimating) advanceStep();
  };
  ...
  const makeAskASKBHandler = useCallback((componentId) => {
    return () => {
      ...
      appendMsg({ type: 'system-query', sourceCount: config.sourceCount, queryText: config.queryText, label });

      const synthId = nextId();
      setTimeout(() => {
        setMessages(prev => [...prev, { id: synthId, type: 'askb-synthesis', label, parts: null, streamingParts: [] }]);
        ...
        const tick = () => {
          const part = flatParts[partIdx];
          if (!part) {
            setMessages(prev => prev.map(m => m.id === synthId ? { ...m, parts: config.parts, streamingParts: null } : m));
            setStreamingMsgId(null);
            setActiveQueryComponents(prev => { const next = new Set(prev); next.delete(componentId); return next; });
            return;
          }
          wordIdx++;
          if (wordIdx >= part.words.length) {
            completedParts.push({ text: config.parts[partIdx].text, chips: config.parts[partIdx].chips });
            setMessages(prev => prev.map(m => m.id === synthId ? { ...m, streamingParts: [...completedParts] } : m));
            partIdx++;
            wordIdx = 0;
            setTimeout(tick, 18);
          } else {
            const currentText = part.words.slice(0, wordIdx).join(' ');
            setMessages(prev => prev.map(m => m.id === synthId ? { ...m, streamingParts: [...completedParts, { text: currentText, chips: [] }] } : m));
            setTimeout(tick, 18);
          }
        };
        setTimeout(tick, 18);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      ...
      <div className="step-indicator">
        {stepLabel && <span className="step-count">{stepLabel}</span>}
        <button
          className="next-btn"
          onClick={advanceStep}
